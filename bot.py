...
CARTS_FILE = os.path.join(BASE_DIR, "user_carts.json")
CONFIG_FILE = os.path.join(BASE_DIR, "bot_config.json")
CHANNELS_FILE = os.path.join(BASE_DIR, "user_channels.json")
...
def parse_post_text(text):
    ...
    for l in lines:
        ...
        if "كود" in cl:
            d = re.findall(r'\d+', l)
            if d: code = f" (كود {d[0]})"
        if any(k in cl for k in ["اسم الموديل", "الموديل", "برا", "اندر", "هاف", "شراب", "بجامه", "بيجامه", "بنطلون", "طقم", "عبايه", "كاش", "ترنج", "فستان", "شورت", "قميص", "كوليكشن", "سوكت", "كلون", "ماركه", "ماركة"]):
    ...
    for l in lines:
        cl = clean_str(l)
        if "سعر الدسته" in cl or "الدسته" in cl or "سعر الدستة" in cl:
    ...
    for l in lines:
        cl = clean_str(l)
        if "السعر" in cl or "سعر" in cl or "ج" in cl or "جنيه" in cl:
            d = re.findall(r'\d+(?:\.\d+)?', cl)
            if d:
                val = float(d[0])
                if val > 30:
...
if __name__ == "__main__":
    ...
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND & filters.ChatType.PRIVA
