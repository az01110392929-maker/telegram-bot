...
def set_user_channel(uid, link):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("INSERT OR REPLACE INTO user_channels (uid, channel_link) VALUES (?, ?)", (str(uid), link))
    conn.commit()
    conn.close()
...
def get_user_channel(uid):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT channel_link FROM user_channels WHERE uid = ?", (str(uid),))
    row = c.fetchone()
    conn.close()
    return row[0] if row else DEFAULT_CHANNEL_LINK

def db_save_product(pid, data):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('''INSERT OR REPLACE INTO products (pid, title, price, doz_price, min_qty, photo_id, link)
                 VALUES (?, ?, ?, ?, ?, ?, ?)''',
              (str(pid), data['title'], data['price'], data['doz_price'], data['min_qty'], data.get('photo_id'), data.get('link')))
    conn.commit()
    conn.close()
...
def db_get_product(pid):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT title, price, doz_price, min_qty, photo_id, link FROM products WHERE pid = ?", (str(pid),))
    row = c.fetchone()
    conn.close()
    if row:
        return {'title': row[0], 'price': row[1], 'doz_price': row[2], 'min_qty': row[3], 'photo_id': row[4], 'link': row[5]}
    return None

def db_add_to_cart(uid, item):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('''INSERT INTO cart_items (uid, title, qty, label, price, doz_price, total, link, photo_id)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''',
              (str(uid), item['title'], item['qty'], item['label'], item['price'], item['doz_price'], item['total'], item['link'], item.get('photo_id')))
    conn.commit()
    conn.close()
...
def db_get_cart(uid):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT id, title, qty, label, price, doz_price, total, link, photo_id FROM cart_items WHERE uid = ?", (str(uid),))
    rows = c.fetchall()
    conn.close()
    return [{'id': r[0], 'title': r[1], 'qty': r[2], 'label': r[3], 'price': r[4], 'doz_price': r[5], 'total': r[6], 'link': r[7], 'photo_id': r[8]} for r in rows]
...
def parse_post_text(text):
    ...
    code_match = re.search(r'(?:كود|الكود|موديل|Model|model)\s*[:\-]?\s*([A-Za-z0-9\-_]+)', text_clean, flags=re.IGNORECASE)
    if code_match:
        code = f" (كود {code_match.group(1)})"

    for l in lines:
        cl = clean_str(l)
        if any(k in cl.lower() for k in ["سوكت", "شيفون", "كلون", "كولون", "اطفال", "شراب", "ليجن", "كارينا", "شورت", "ماركه", "ماركة", "aisha", "اسم الموديل", "الموديل", "موديل", "برا", "اندر", "هاف", "بجامه", "بيجامه", "بنطلون", "طقم", "عبايه", "كاش", "ترنج", "فستان", "قميص", "كوليكشن"]):
            if not title:
...
async def send_cart_view(bot, chat_id, uid):
    ...
    await bot.send_message(chat_id=chat_id, text=summary, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=ParseMode.HTML, disable_web_page_preview=True)
