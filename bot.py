...
BOT_TOKEN = "8896922203:AAFhJCEcN4Gsa2WwsjIfZoXuzHK8Acj5Txs"
BOT_USERNAME = "Fyteeuyeewwcjuwbot"
...
def parse_post_text(text):
    ...
    title, code, min_qty, unit_price, doz_price = "", "", 1, 0.0, 0.0
...
def get_quantity_label(qty):
    ...
    doz = qty // 12
    rem = qty % 12
    if rem == 0: return f"{doz} دسته ({qty} قطعة)"
    elif rem == 3: return f"{doz} دسته وربع ({qty} قطعة)"
    elif rem == 6: return f"{doz} دسته ونصف ({qty} قطعة)"
    elif rem == 9: return f"{doz + 1} دسته إلا ربع ({qty} قطعة)"
    return f"{qty} قطعة"

def generate_quantity_keyboard(post_id, min_qty):
    kb = []
    if min_qty >= 12:
        q_list = [(12,"1 دسته"),(24,"2 دسته"),(36,"3 دسته"),(48,"4 دسته"),(60,"5 دسته"),(72,"6 دسته")]
    elif min_qty == 6:
        q_list = [(6,"نص دسته"),(12,"1 دسته"),(18,"دسته ونص"),(24,"2 دسته"),(36,"3 دسته"),(48,"4 دسته")]
    else:
        q_list = [(1,"1 قطعة"), (3,"ربع دسته"),(6,"نص دسته"),(9,"دسته إلا ربع"),(12,"1 دسته"),(15,"دسته وربع"),(18,"دسته ونص"),(24,"2 دسته")]
    ...
    kb.append([InlineKeyboardButton("✍️ كتابة كمية اخري", callback_data=f"custom_{post_id}")])
    ...
    if data:
        ...
        try:
            await context.bot.edit_message_reply_markup(
                ...
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🛍️ تسوق واطلب هذا الموديل", url=f"https://t.me/{BOT_USERNAME}?start=buy_{pid}")]])
            )
...
async def send_cart_view(bot, chat_id, uid, is_after_delete=False):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("❌ حذف صنف", callback_data="manage_items_after" if is_after_delete else "manage_items")],
...
async def view_cart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    await send_cart_view(context.bot, update.effective_chat.id, str(update.effective_user.id), is_after_delete=False)
...
async def send_wa_and_clear(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_carts[uid] = []
    save_data(CARTS_FILE, user_carts)
    await query.message.reply_text("✅ <b>تم تأكيد الأوردر بنجاح!</b>", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📲 اضغط هنا لفتح الواتساب وإرسال الفاتورة", url=wa_link)]]), parse_mode=ParseMode.HTML)
...
async def manage_items(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    m_head = await query.message.reply_text("🗑️ <b>اختر الصنف للحذف:</b>", parse_mode=ParseMode.HTML)
    sent_delete_messages[uid] = [m_head.message_id]
    
    for idx, it in enumerate(cart, 1):
        cap = f"❌ صنف ({idx}): <b>{html.escape(it['title'])}</b>"
        kb = InlineKeyboardMarkup([[InlineKeyboardButton(f"❌ حذف رقم {idx}", callback_data=f"del_{idx-1}")]])
...
async def delete_single_item(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 0 <= idx < len(user_carts[uid]):
        ...
        save_data(CARTS_FILE, user_carts)
        await query.message.reply_text(f"🗑️ تم الحذف بنجاح!")
...
async def clear_cart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_carts[uid] = []
    save_data(CARTS_FILE, user_carts)
    await query.message.reply_text("تم تفريغ الفاتورة بنجاح ✅", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع للقناة", url=get_user_channel(uid))]]))
...
if __name__ == "__main__":
    ...
    print("البوت يعمل الآن بصورة سليمة تماماً...")
    app.run_polling()
